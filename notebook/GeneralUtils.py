...
class GeneralUtils(object):
    ...
    def descriptive_statistics(self, data_set):

        # central tendency: mean, median
        mean_value = pd.DataFrame(data_set.apply(np.mean)).T
        median_value = pd.DataFrame(data_set.apply(np.median)).T

        # distribution: std, min, max, range, skew, kurtosis
        std_value = pd.DataFrame(data_set.apply(np.std)).T
        ...
        skewness_value = pd.DataFrame(data_set.apply(lambda x: x.skew())).T
        kurtosis_value = pd.DataFrame(data_set.apply(lambda x: x.kurtosis())).T

        # concatenates
        summary_stats = pd.concat([min_value, max_value, range_value, mean_value, median_value, std_value,
                                   skewness_value, kurtosis_value]).T.reset_index()
